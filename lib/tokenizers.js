...
let TOKENIZER_RECORD_CIPHER = null;

bedrock.events.on('bedrock.init', async () => {
  // initialize tokenizer record cipher
  const {
    tokenizerRecordEncryption: encryptionCfg
  } = bedrock.config.tokenizer;

  const options = {
    currentKekId: encryptionCfg?.currentKekId ?? null,
    keks: encryptionCfg?.keks ?? [],
    encoding: 'cbor'
  };
  TOKENIZER_RECORD_CIPHER = await RecordCipher.create(options);
  const status = TOKENIZER_RECORD_CIPHER.isSecretsEncryptionEnabled() ?
    'enabled' : 'disabled';
  logger.info(`Tokenizer record encryption is ${status}.`);
});
...
async function _tokenizerFromRecord({record}) {
  // decrypt any record secrets
  record = await TOKENIZER_RECORD_CIPHER.decryptRecordSecrets({record});
  if(record.secrets?.tokenizerSecret) {
    record.tokenizer.secret = record.secrets.tokenizerSecret;
  }

  const {tokenizer} = record;
  // 1. Generate capability agent from handle and secret.
  const handle = 'primary';
  const secret = Buffer.from(tokenizer.secret, 'base64url');
...
export async function _createTokenizer() {
  try {
    ...
    // encrypt tokenizer secret if enabled
    if(TOKENIZER_RECORD_CIPHER.isSecretsEncryptionEnabled()) {
      record.secrets = {tokenizerSecret: tokenizer.secret};
      record.tokenizer = {...tokenizer};
      delete record.tokenizer.secret;
    }
    try {
